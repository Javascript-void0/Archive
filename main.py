...
async def update_embed(page, dir, message, lines):
    title, body = lines[page].split(" SPLIT ")
    embed = discord.Embed(title=f'{dir}. Document', description=f'Page {page+1}')
    ...
    with open(f'index/{dir}.txt', 'r') as file:
        ...
        while True:
            try:
                ...
                if reaction.emoji == '⏮':
                    page = 0
                    await update_embed(page, dir, message, lines)
